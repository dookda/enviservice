<?php
if($usertype=='1'||$usertype=='2'&&in_array("edit_list", $perm_data)){}else{ die('ไม่อนุญาตให้เข้าถึงข้อมูลนี้');}
?>
<style>
...
<form role="form" id="add_service_form" method="post" enctype="multipart/form-data">
        	...
        	<div class="box-body text-right">
               ...
               <div class="btn-group"><p><label><input type="checkbox" name="sendmail"> แจ้งเตือนลูกค้าด้วย </label></p></div> 
...
jQuery(document).ready( function($) {
	...
	$('.addlist').on('click',function(){
		...
		if(text!=''){
			...
			$('#list_area').append('<div style=" background: #fff; border-bottom: 2px solid #eeeeee; padding: 10px; position: relative;" class="ui-state-default list_data" id="list'+list_data_num+'"><span style="right: 5px; top: 8px;" class="img_close" onclick="$(this).dellist(\'list'+list_data_num+'\')" ><i class="fa fa-times"></i></span>'+text+'<input type="hidden" name="list[]" value="'+text+'"/></div>');
			$('.list_input').val('');
