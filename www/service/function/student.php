...
function check_email($val){
	global $ConnectDB;
	$sql="SELECT `email` FROM `users` WHERE `email` = '".$val."' LIMIT 0 , 1";
	$result=mysqli_query($ConnectDB,$sql);
	$nums = mysqli_num_rows($result);
	return $nums;
}
function check_edit_email($val,$id){
	global $ConnectDB;
	$sql="SELECT `email` FROM `users` WHERE `email` = '".$val."' AND `id`!='".$id."' LIMIT 0 , 1";
	$result=mysqli_query($ConnectDB,$sql);
	$nums = mysqli_num_rows($result);
	return $nums;
}

function check_select($val,$cur){
